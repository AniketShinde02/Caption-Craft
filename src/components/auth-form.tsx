...
export function AuthForm({ initialEmail = '' }: { initialEmail?: string }) {
  ...
  const handleForgotPassword = async () => {
    ...
    try {
      ...
      } else if (data.type === 'email_not_found') {
        setForgotPasswordError('This email is not registered. Please click "Sign Up" above to create an account.');
        // Automatically switch to sign-up tab to help the user
        setTimeout(() => {
          handleTabChange('sign-up');
        }, 2000); // Switch after 2 seconds to give user time to read the message
  ...
  return (
    <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
            ...
            <Form {...signInForm}>
              ...
              >
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <Input
                          ...
                          className="bg-background/80 h-10 sm:h-9 text-sm sm:text-base focus-visible:ring-0 focus-visible:ring-offset-0"
                          {...field}
                        />
                      </FormControl>
                      ...
                      <FormControl>
                        <div className="relative">
                          <Input
                            ...
                            placeholder="••••••••"
                            className="bg-background/80 pr-12 h-10 sm:h-9 text-sm sm:text-base focus-visible:ring-0 focus-visible:ring-offset-0"
                            {...field}
                          />
                          <button
                            type="button"
                            onClick={() => setShowSignInPassword(!showSignInPassword)}
                            className="absolute inset-y-0 right-0 flex items-center pr-3 text-muted-foreground hover:text-foreground transition-colors focus-visible:ring-0 focus-visible:ring-offset-0"
                          >
                ...
                <div className="text-right">
                  <button
                    type="button"
                    className="text-sm text-muted-foreground hover:text-primary transition-colors py-2 focus-visible:ring-0 focus-visible:ring-offset-0"
                ...
                {forgotPasswordError && (
                  <div className="space-y-1">
                    ...
                    {forgotPasswordError.includes('not registered') && (
                      <p className="text-xs text-muted-foreground text-center px-2">
                        💡 Click "Sign Up" tab above to create account
                      </p>
                    )}
                  </div>
                )}
                ...
                <Button type="submit" disabled={isLoading} className="w-full h-10 sm:h-9 text-base sm:text-sm focus-visible:ring-0 focus-visible:ring-offset-0">
                  {isLoading ? (
      ...
      <TabsContent value="sign-up" className="mt-3 sm:mt-4">
        <Card className="bg-transparent border-none shadow-none">
          <CardContent className="p-0">
            <Form {...signUpForm}>
              <form
