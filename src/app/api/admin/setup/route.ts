...
export async function POST(request: NextRequest) {
  try {
    ...
    // Early return for simple actions that don't need database connection
    if (action === 'verify-token') {
      ...
      try {
        // Verify JWT token first (fast operation)
        ...
        // Check if JWT token has already been used (by JTI) - this is the only DB operation needed
        const isUsed = await isTokenUsed(decoded.jti);
        if (isUsed) {
