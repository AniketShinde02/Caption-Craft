...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error.message?.includes('AI generation failed') || 
        error.message?.includes('Failed to save captions')) {
      return NextResponse.json(
        { 
          success: false, 
          message: 'Unable to generate captions at this time. Please try again with a different image.',
    ...
    if (error.message?.includes('Unexpected token') || 
      ...
      return NextResponse.json(
        { 
          success: false, 
          message: 'Technical issue occurred while processing the response. Please try again.',
          type: 'parsing_error'
        },
        { status: 500 }
      );
